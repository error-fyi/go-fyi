...
func (j *Error) UnmarshalJSON(b []byte) error {
	...
	if len(plain.Code) >= 40 {
		return fmt.Errorf("field %s length: must be <= %d", "code", 40)
	}
	if len(plain.Short) >= 70 {
		return fmt.Errorf("field %s length: must be <= %d", "short", 70)
	}
	if len(plain.Title) >= 40 {
		return fmt.Errorf("field %s length: must be <= %d", "title", 40)
	}
	*j = Error(plain)
	return nil
}
